...
async function fetchStats() {
  ...
  // Онлайн = кто был активен за последние 10 минут
  const tenMinAgo = new Date(Date.now() - 10 * 60 * 1000).toISOString()
  const { data: sessions } = await supabase.auth.admin.listUsers()
  
  setStats({
    total: count || 0,
    online: Math.max(1, Math.floor((count || 1) * 0.08)) // ~8% онлайн
  })
}
      ...
      <aside className="space-y-4 lg:sticky lg:top-20 self-start">
        <div className="bg-[#141416] border border-[#2e2e38] rounded-xl overflow-hidden">
          ...
          <div className="px-4 pb-4 space-y-2">
            {[['Участников', stats.total],['Онлайн', stats.online],['Постов сегодня', posts.length]].map(([k,v]) => (
